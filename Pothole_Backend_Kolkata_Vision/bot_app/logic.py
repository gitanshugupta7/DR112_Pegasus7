...
def Get_Ward(my_lat,my_long):
    with open('C:/Users/GITANSHU/DjangoAPI/Pothole_Backend_Kolkata_Vision/kolkata.geojson') as f:
        js = json.load(f)
...
""", use_aliases=True)
        ...
        local_file = open("C:/Users/GITANSHU/DjangoAPI/Pothole_Backend_Kolkata_Vision/media/"+ str(final[key]['image_id']) + ".png", 'wb')
        ...
        if(punk==0):
            # os.remove("C:/Users/GITANSHU/DjangoAPI/Pothole_Backend_Kolkata_Vision/media/"+ str(final[key]['image_id']) + ".jpg")
            client = Client(account_sid, auth_token)
            response = emoji.emojize("""
...
""", use_aliases=True)
        ...
        if(punk==1):
            check = resize.resize_image(unique_id)
            print("Check value is ",check)
            if(check==1 and punk==1):
                client = Client(account_sid, auth_token)
                response = emoji.emojize("""
You have uploaded a downloaded image of the pothole! :cross_mark:

You have to click the image of the pothole and then upload the image of the same.
""", use_aliases=True)
                message = client.messages \
                    .create(
                        from_='whatsapp:+14155238886',
            ...
            if((final[key]['name'] is not None) and (final[key]['latitude'] is None or final[key]['longitude'] is None) and (punk == 1) and (check==0)):
...
""", use_aliases=True)
            ...
            elif((final[key]['name'] is None) and (final[key]['latitude'] is None or final[key]['longitude'] is None) and (punk == 1) and (check==0)):
                final[key]['media_url'] = image_url
                print("Case 2")
                client = Client(account_sid, auth_token)
                response = emoji.emojize("""
...
""", use_aliases=True)
            ...
            elif((final[key]['name'] is None) and (final[key]['latitude'] is not None or final[key]['longitude'] is not None) and (punk == 1) and (check==0)):
                final[key]['media_url'] = image_url
                print("Case 3")
                client = Client(account_sid, auth_token)
                response = emoji.emojize("""
...
""", use_aliases=True)
            ...
            elif((final[key]['name'] is not None) and (final[key]['latitude'] is not None or final[key]['longitude'] is not None) and (punk == 1) and (check==0)):
                final[key]['media_url'] = image_url
                print("Case 4")
                client = Client(account_sid, auth_token)
                response = emoji.emojize("""
